...
public class DialogComponentMultiFileChooser extends DialogComponent {
	 ...
	 * 
	 * @param noColoring
	 *            if set true, the component will not be marked red, even if the
	 *            entered value was erroneous.
	...
	private void updateModel() throws InvalidSettingsException {
		 ...
		 * 
		 * try { ((SettingsModelString)getModel()).setStringValue(file); } catch
		 * (final RuntimeException e) { // if value was not accepted by setter
		 * method if (!noColoring) { showError(m_fileComboBox); } throw new
		 * InvalidSettingsException(e); }
		 * 
		 * } else { if (!noColoring) { showError(m_fileComboBox); } throw new
		 * InvalidSettingsException("Please specify a filename."); }
		 */
	...
	@Override
	protected void checkConfigurabilityBeforeLoad(PortObjectSpec[] arg0)
