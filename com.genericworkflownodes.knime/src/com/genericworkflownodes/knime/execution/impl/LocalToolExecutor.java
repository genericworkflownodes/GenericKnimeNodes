...
public class LocalToolExecutor implements IToolExecutor {
     ...
     */
    private void setupProcessEnvironment(final ProcessBuilder builder) {
        for (final String key : m_environmentVariables.keySet()) {
            final String value = expandEnvironmentVariables(
                    m_environmentVariables.get(key));
     ...
     */
    private void findExecutable(final INodeConfiguration nodeConfiguration,
            final IPluginConfiguration pluginConfiguration)
                    throws NoBinaryAvailableException {
        m_executable = pluginConfiguration.getBinaryManager()
                .findBinary(nodeConfiguration.getExecutableName());
    }
